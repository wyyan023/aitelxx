...
class Manager extends User{
	 ...
	 * @return string 
	*/
  protected function get_user_role_id($user_id)
  {
    if(empty($user_id)){
      return false;
    }
    $role_id = Db::name('admin')
                ->where('id', $user_id)
                ->value('role_id');
    return $role_id;
  }
    ...
    $user_role_id = $this->get_user_role_id($user_info['uid']);
		...
		//获取角色列表
		$mlist = Db::name('admin_role')->field('id,name')->where('status', 1)->where('id', '>', $user_role_id)->order('id asc')->select();		
