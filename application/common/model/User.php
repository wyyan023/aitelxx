...
class User extends Base{
	...
	* 用户登录模型
	*/
	public function login($username = '', $password = '', $type = 1,$isautologin=1, $location= '', $userType = 0){
		$map = array();
		...
		if($user && $user['status']){
			...
			if(md5($password.$user['salt']) === $user['password']){
				...
				if ($location){
					...
					$user['longitude'] = $location[0];
					$user['latitude'] = $location[1];
				}
	...
	function register($mobile, $password, $username='',$userType=0, $email='', $inviteCode =''){
		...
		$result = db('member')->insertGetId($data);//$this->validate(false)->save($data);
		if ($result) {	
			//添加邀请记录
			if (isset($userInfo)){	
