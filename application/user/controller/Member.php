...
class Member extends User{
	...
	public function memberListAjax(){
		...
		$where["user_type"] = 0;  //白名单   0是普通用户 1是白名单用户
		...
		// if (is_array($knlgLabels) && count($knlgLabels)){
		// 	$knlgLabelsStr = ',('.implode('|', $knlgLabels).'),';
		// 	if (is_array($semanticLabels) && count($semanticLabels) || is_array($flowLabels) && count($flowLabels)){
		// 		$subWhere .= " and concat(',',knowledge_label,',') regexp '".$knlgLabelsStr."'";
		// 	}
		// 	else{
		// 		$subWhere = "concat(',',knowledge_label,',') regexp '".$knlgLabelsStr."'";
		// 	}
			
		// }
		
		// if($call_times){
		// 	$where["call_times"] = $call_times;
		// }
		
		// if($affirm_times){
		// 	$where["affirm_times"] = $affirm_times;
		// }
		// if($negative_times){
		...
		$list = Db::name('member')
			...
			->select();
			/*
			s.name as scenarios_name,c.task_name,g.name,
		...
		foreach ($list as $k=>$v){
			...
			// $list[$k]['task_name'] = $TelConfig->get_task_name($v['task']);
			// if(!$v["task_name"]){
			// 	$list[$k]["task_name"] = "--";
			// }
			...
			// 	$list[$k]["flow_label"] = "--";
			// }
			// $list[$k]['scenarios_name'] = $TelScenarios->get_name($v['scenarios_id']);
			// if(!$v["scenarios_name"]){
  ...
  public function backdetail(){
    	...
    	if($froms == 'record'){
            ...
            $recordId = input('recordId','','trim,strip_tags');
						...
						->field('uid,mobile,nickname,status,level,sex,duration,last_dial_time,record_path,call_id,call_times,originating_call,call_rotation')
						->where('mobile',$mobile)
						->where('task',$taskId)
						->find();
						$Info = Db::name('tel_call_record')
							->field('status,level,duration,last_dial_time,call_id,call_times,originating_call,record_path')
							->where('id',$recordId)
							->find();
						...
						if(count($memberInfo)){
							$review = array();
							$review['review'] = 1;
							Db::name('tel_call_record')->where('id', $recordId)->update($review);
						}
    	}else{
				
    		$memberInfo = Db::name('member')
											->field('mobile,nickname,status,level,sex,duration,last_dial_time,record_path,call_id,call_times,originating_call,call_rotation')
											->where('uid',$uid)
											->find();
			...
			$num = Db::name('tel_bills')->where('status',1)->where('call_id',$memberInfo['call_id'])->count(1);
			...
			// if (isset($memberInfo['sex']) === true && $memberInfo['sex'] >= 0){
			// 	$memberInfo['sex'] = $memberInfo['sex']?'女':'男';
			// }
			// else{
			// 	$memberInfo['sex'] = '未知';
    ...
    public function add_call()
    {
    	if(IS_POST){
	    	...
	    	if(empty($user_auth['uid'])){
	    		return returnAjax(1, '未登陆');
	    	}
	    	//话术ID
	    	$scenarios_id = input('scenarios_id', '', 'trim,strip_tags');
	    	//线路ID
	    	$call_phone_id = input('call_phone_id', '', 'trim,strip_tags');
	    	//号码
	    	$phone = input('phone', '', 'trim,strip_tags');
	    	if(empty($scenarios_id) || empty($call_phone_id) || empty($phone)){
	    		return returnAjax(1, '参数错误');
	    	}
	    	//验证话术是否存在
	    	$result = Db::name('tel_scenarios')
	    	...
	    	$run_robot_cnt = $AdminData->get_run_robot_count($user_auth['uid']);
	    	if(($usable_robot_cnt - $run_robot_cnt) < 1){
	    		return returnAjax(2, '机器人数量不足');
	    	}
	    	$PlanData = new PlanData();
	    	$result = $PlanData->add_plan($user_auth['uid'], $scenarios_id, $call_phone_id, $phone);
	    	if($result === true){
	    		return returnAjax(0, '成功');
	    	}
    ...
    //新的呼叫列表
     ...
     //ajax 返回通话记录列表
		public function callLog(){
			...
			if($negative_times){
				$where["negative_times"] = $negative_times;
			}
			
			if($neutral_times){
				$where["neutral_times"] = $neutral_times;
			}
			
			if($effective_times){
				$where["effective_times"] = $effective_times;
			}
			
			if($hit_times){
				$where["hit_times"] = $hit_times;
			}
