		...
		if (IS_POST) {
			...
			if ($taskresult){
				...
				if(!empty($import_number_args)){
					...
					//意向标签
					if(!empty($import_number_args['level'])){
						$member_where['level'] = ['in', explode(',', $import_number_args['level'])];
					}
					//通话状态
					if(isset($import_number_args['status']) === true && !empty($import_number_args['status'])){
					...
					Log::info(json_encode($member_where));
					$data = Db::name('member')
									->field('mobile')
									->group('mobile')
									->where($member_where)
  ...
  // 修改任务状态
    ...
    //新的编辑计划,获取任务信息
		public function editplanInfo(){
				...
				//获取配置表的信息
				$cfresult = Db::name('tel_config')
							->field('id,level,sale_ids,scenarios_id,destination_extension,phone,call_type,bridge,robot_cnt,call_phone_id')
							->where('task_id', $id)->find();	
