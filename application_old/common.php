...
// RuiKeCMS常量定义
define('RuiKeCMS_VERSION', '3.0.20160630');
...
function authcode($string, $operation = 'DECODE', $key = '', $expiry = 0) {
	...
	// 当此值为 0 时，则不产生随机密钥
	$uc_key = config('data_auth_key') ? config('data_auth_key') : 'ruikecms';
...
function get_cover($cover_id, $field = null) {
	...
	if ($qiniuUpload['open'] && $picture['path']){
		$picture['path'] = $qiniuUpload['qiniuUrl'].$picture['path'];
	}
 ...
 * @author huajie <banhuajie@163.com>
 */
function get_image($cover_id, $field = null) {
	if (empty($cover_id)) {
		return '';//BASE_PATH . '/public/images/default.png';
	}
	
	$picture = db('picture',[], false)->where( 'id', $cover_id)->find();
	...
	$qiniuUpload = config('qiniu_upload');
	if ($qiniuUpload['open']){
		$imgUrl = $qiniuUpload['qiniuUrl'].$picture['path'];
	}
	
	$aliyunUpload = config('aliyun_upload');
	if ($aliyunUpload['open']){
		$imgUrl = $aliyunUpload['aliYunOssUrl'].$picture['path'];
	}
 ...
 * @author huajie <banhuajie@163.com>
 */
function get_upload_file($file_id, $field = null) {
	if (empty($file_id)) {
		return '';
	}
	$file = db('File')->where(array('id' => $file_id))->find();
	...
	$qiniuUpload = config('qiniu_upload');
	if ($qiniuUpload['open']){
		$fileUrl = $qiniuUpload['qiniuUrl'].$fileUrl;
	}
...
function & load_wechat($type = '', $wxConfig=array()) {
     static $wechat = array();
    ...
    if (!isset($wechat[$index])) {
	        ...
	        // @TODO：查询出记录在数据表的参数   
	        $token = input('token','','trim,strip_tags');    
	        $config = db('wx_user')->field('token, appid, appsecret, encodingaeskey, mch_id, partnerkey, ssl_cer, ssl_key')->where('token',$token)->find();
				}
        // @TODO：设置SDK缓存路径（根据自己项目实际情况设置目录）
        $config['cachepath'] = CACHE_PATH . 'wx/';
...
//根据ip地址获取区域
function getAreaByIp($clientIP){
	$host = "http://jisuip.market.alicloudapi.com";
	$path = "/ip/location";
	$method = "GET";
	$appcode = "13fdb53908f34917a566b8468dc6668a";
	$headers = array();
	...
	$result = json_decode($IPinfo,true)["result"];

	\think\Log::record('clientIP:'.$clientIP);
	\think\Log::record('info:'.$IPinfo);
...
function sendTemplateMsg($touser, $template_id, $url='', $data, $topcolor = '#173177',$wxInfo=""){
		$template = array(
        'touser' => $touser,
    ...
    if (!$wxInfo){
  	...
  	$receive = & load_wechat('Receive', $wxInfo);
  	$ret = $receive->sendTemplateMessage($template);
  	//\think\Log::record('sendTemplateMessage failure='.$ret);
}

//发送客服消息    	
function sendKeFuMsg($wxInfo, $openId, $msg){		
	$sendMsgObj = & load_wechat('Receive', $wxInfo);
	$data["touser"] = $openId;
  $data["msgtype"] ="text";
	$data["text"] = array(
 			"content"=>$msg
	...
	\think\Log::record('#####sendKeFuMsg='.json_encode($data).'###='.json_encode($ret));
}
